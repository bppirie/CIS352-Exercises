...
(define/contract (next-betas e)
  ...
  (match e
    ...
    [`(lambda (,x) ,e-body) (define e-body-betas (next-betas e-body))
                            (foldl (lambda (e-next acc) (set-add acc `(lambda (,x) ,e-next))) (set) (set->list e-body-betas))]
    [`((lambda (,x) ,e-body) ,e-arg) (define e-arg-betas (next-betas e-arg))
                                     (set-union (foldl (lambda (e-next acc) (set-add acc `((lambda (,x) ,e-next) ,e-arg)))
                                                       (set)
                                                       (set->list (next-betas e-body)))
                                                (set-add
                                                 (foldl (lambda (e-next acc) (set-add acc `((lambda (,x) ,e-body) ,e-next)))
                                                        (set)
                                                        (set->list e-arg-betas))
                                                 (subst e-body x e-arg)))]
    [`(,e0 ,e1) (define e0-betas (next-betas e0))
                (define e1-betas (next-betas e1))
                (set-union (foldl (lambda (e-next acc) (set-add acc `(,e-next ,e1)))
                                  (set)
                                  (set->list e0-betas))
                           (foldl (lambda (e-next acc) (set-add acc `(,e0 ,e-next)))
                                  (set)
                                  (set->list e1-betas)))]))
